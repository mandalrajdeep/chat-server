...
public class CustomerExecutive implements Comparable<CustomerExecutive> {
    ...
    /**
     * Sort the executive based on their availability and the count of active
     * customers being handled
     */
    @Override
    public int compareTo(CustomerExecutive o) {
        if (this.status.equals("available")
                && o.getStatus().equals("available")) {
            if (this.customersCount < o.getCustomersCount()) {
                ...
                && o.getStatus().equals("unavailable")) {
            return -1;
        } else {
            return 1;
        }
    ...
    public String toString() {
        return "CustomerExecutive [username=" + this.username
                + ", customersCount=" + this.customersCount + ", status="
                + this.status + "]";
    }
    ...
    public boolean equals(Object obj) {
        if (obj != null && obj instanceof CustomerExecutive) {
            return (this.username.equals(((CustomerExecutive) obj)
                .getUsername()));
        }
        return false;
    }
